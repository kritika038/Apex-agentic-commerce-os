...
def generate_marketplace_products() -> List[Dict[str, Any]]:
    ...
    category_data = [
        # 1. Sports & Fitness
        ("Sports & Fitness", "Running Shoes", "Nike", "Nike Air Zoom Pegasus 40", "Premium daily trainer with responsive Zoom Air cushioning.", Decimal("4799.00"), Decimal("5999.00"), 4.7, 340, "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Running Shoes", "Adidas", "Adidas Ultraboost Light 23", "Iconic energy-returning Boost midsole running shoe for long distance.", Decimal("5499.00"), Decimal("6999.00"), 4.8, 412, "https://images.unsplash.com/photo-1587563871167-1ee9c731aefb?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Training Shoes", "Puma", "Puma Fuse 2.0 Cross Training Shoes", "Engineered stability shoe for heavy deadlifts and functional workouts.", Decimal("3299.00"), Decimal("4299.00"), 4.5, 120, "https://images.unsplash.com/photo-1608231387042-66d1773070a5?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Football Shoes", "Nike", "Nike Mercurial Vapor 15 Club", "Moulded synthetic upper speed cleats with turf studs.", Decimal("3899.00"), Decimal("4999.00"), 4.6, 95, "https://images.unsplash.com/photo-1511556532299-8f662fc26c06?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Basketball Shoes", "Nike", "Nike Precision 6 Low Basketball", "Quick-cut grip traction and sculpted foam midsole for court control.", Decimal("4499.00"), Decimal("5499.00"), 4.4, 76, "https://images.unsplash.com/photo-1579338559194-a162d19bf842?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Cricket Shoes", "Asics", "Asics Gel-Peake Cricket Spikes", "Reinforced toe protection and GEL shock absorption for turf wickets.", Decimal("4999.00"), Decimal("6499.00"), 4.6, 64, "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Gym Gloves", "Decathlon", "Domyos Weightlifting Grip Gloves", "Anti-callus breathable palm padding with wrist wrap support.", Decimal("499.00"), Decimal("699.00"), 4.3, 180, "https://images.unsplash.com/photo-1583473848882-f9a5bc7fd2ee?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Yoga Mats", "Decathlon", "Kimjaly 8mm High-Grip Yoga Mat", "Non-slip eco-friendly textured TPE yoga and pilates exercise mat.", Decimal("999.00"), Decimal("1499.00"), 4.7, 230, "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Resistance Bands", "Decathlon", "Core Strength Resistance Band Set (5-Pack)", "Natural latex loop exercise bands from 5kg to 30kg resistance.", Decimal("699.00"), Decimal("999.00"), 4.5, 310, "https://images.unsplash.com/photo-1598289431512-b97b0917affc?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Dumbbells", "Decathlon", "Cast Iron Hex Dumbbell Pair 5kg", "Anti-roll rubber-coated hex dumbbells for strength conditioning.", Decimal("1799.00"), Decimal("2299.00"), 4.8, 140, "https://images.unsplash.com/photo-1586401100295-7a8096fd231a?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Kettlebells", "Decathlon", "Competition Cast Iron Kettlebell 12kg", "Ergonomic wide grip handle for kettlebell swings and squats.", Decimal("2199.00"), Decimal("2799.00"), 4.7, 85, "https://images.unsplash.com/photo-1584735935682-2f2b69dff9d2?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Skipping Ropes", "Nike", "Nike Speed Skipping Jump Rope", "Smooth bearing fast cable jump rope with adjustable steel wire.", Decimal("799.00"), Decimal("1199.00"), 4.4, 190, "https://images.unsplash.com/photo-1598289431512-b97b0917affc?w=600&auto=format&fit=crop&q=80", True),
        ("Sports & Fitness", "Shakers", "Puma", "Puma Pro Protein Shaker Bottle 700ml", "BPA-free leakproof shaker with stainless steel blending whisk ball.", Decimal("349.00"), Decimal("499.00"), 4.5, 270, "https://images.unsplash.com/photo-1602143407151-7111542de6e8?w=600&auto=format&fit=crop&q=80", True),
        ...
        ("Sports & Fitness", "Track Pants", "Adidas", "Adidas Tiro 23 Training Track Pants", "Moisture-absorbing tapered pants with ankle zips for training.", Decimal("1899.00"), Decimal("2499.00"), 4.7, 185, "https://images.unsplash.com/photo-1552902865-b72c031ac5ea?w=600&auto=format&fit=crop&q=80", True),
    ...
    # Multiply with realistic color/size variants to create a rich marketplace
    expanded_products = list(all_products)
    colors = ["Black", "Navy Blue", "Pure White", "Crimson Red", "Space Grey", "Olive Green"]
    sizes = ["UK 7", "UK 8", "UK 9", "UK 10", "Medium", "Large", "XL", "128GB", "256GB"]

    var_idx = 500
    for base_p in all_products[10:]: # expand non-baseline
        for c_idx in range(2):
            var_idx += 1
            color_choice = colors[(var_idx) % len(colors)]
            size_choice = sizes[(var_idx) % len(sizes)]
            var_p = dict(base_p)
            var_p["name"] = f"{base_p['name']} ({color_choice} - {size_choice})"
            var_p["description"] = f"{base_p['description']} Color: {color_choice}, Specification/Size: {size_choice}."
            var_p["gtin"] = None
            var_p["model_number"] = f"{base_p['model_number']}-{color_choice[:2].upper()}" if base_p.get("model_number") else None
            var_p["sku"] = f"{base_p['sku']}-{var_idx}" if base_p.get("sku") else None
            var_p["variant_group_id"] = base_p.get("model_number")
            
            if base_p.get("subcategory") == "T-Shirts":
                if color_choice == "Pure White":
                    var_p["image_url"] = "https://images.unsplash.com/photo-1581655353564-df123a1eb820?w=600&auto=format&fit=crop&q=80"
                elif color_choice == "Black":
                    var_p["image_url"] = "https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?w=600&auto=format&fit=crop&q=80"
            
            is_apparel = base_p.get("category") in ["Apparel", "Fashion"] and base_p.get("subcategory") in ["T-Shirts", "Jackets", "Jeans", "Shorts", "Track Pants"]
            var_p["attributes"] = {
                "color": color_choice,
                "size": size_choice,
                "vto_image_ready": is_apparel
            }
            delta = Decimal("100.00") if c_idx == 1 else Decimal("0.00")
            var_p["price"] = base_p["price"] + delta
            var_p["mrp"] = base_p["mrp"] + delta if base_p["mrp"] else None
            var_p["stock"] = 25 + (var_idx % 30)
            
            var_offers = []
            for off in base_p.get("external_offers", []):
                v_off = dict(off)
                if v_off.get("price"):
                    v_off["price"] = off["price"] + delta
                if v_off.get("mrp"):
                    v_off["mrp"] = off["mrp"] + delta
                var_offers.append(v_off)
            var_p["external_offers"] = var_offers
            expanded_products.append(var_p)
